...
export default function CustomerManagementPage() {
  ...
  const fetchAll = async () => {
    ...
    try {
      const { data } = await apiClient.get<Customer[]>(endpoints.customers.root(), {
        next: { revalidate: 0 },
      });
      setCustomers(data);
      setTableStatus({
        type: "success",
        message: `Berhasil memuat ${data.length} customer.`,
      });
    } catch (error) {
      setTableStatus({
        type: "error",
        message: parseError(error, "Gagal memuat daftar customer."),
      });
    }
  };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (isEditing) {
      try {
        const { data } = await apiClient.put<Customer>(
          endpoints.customers.byId(selectedId!),
          { json: payload }
        );
        setCustomers((prev) =>
          prev.map((customer) =>
        ...
        setFormStatus({
          type: "success",
          message: `Customer ${selectedId} berhasil diperbarui.`,
        });
      } catch (error) {
        setFormStatus({
          type: "error",
          message: parseError(error, "Gagal memperbarui customer."),
        });
      }
      return;
    }
    ...
    try {
      ...
      setForm({ id: "", name: "", email: "", phone: "", status: "" });
    } catch (error) {
      setFormStatus({
        type: "error",
        message: parseError(error, "Gagal membuat customer."),
      });
    }
  };
  ...
  const handleDelete = async (customerId: string) => {
    ...
    try {
      await apiClient.delete<void>(endpoints.customers.byId(customerId));
      setCustomers((prev) => prev.filter((customer) => customer.id !== customerId));
      if (selectedId === customerId) handleCancelEdit();
      setRowStatus({
        type: "success",
      ...
      setRowStatus({
        type: "error",
        message: parseError(error, "Gagal menghapus customer."),
      });
    }
  };

  const handleRefreshRow = async (customerId: string) => {
    setRowStatus({ type: "loading" });
    try {
      ...
      setRowStatus({
        type: "success",
        message: `Detail customer ${customerId} diperbarui.`,
      });
    } catch (error) {
      setRowStatus({
        type: "error",
        message: parseError(error, "Gagal mengambil detail customer."),
      });
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-amber-50 text-zinc-900 dark:from-black dark:via-zinc-950 dark:to-zinc-900 dark:text-zinc-100">
      <div className="mx-auto flex max-w-6xl flex-col gap-10 px-6 py-12">
        ...
        <header className="space-y-4">
          ...
          <div className="space-y-2">
            <h1 className="text-4xl font-semibold leading-tight">Tabel Customer</h1>
            <p className="max-w-3xl text-base text-zinc-600 dark:text-zinc-300">
              Kelola data customer: lihat, tambah, ubah, dan hapus tanpa token.
            </p>
          </div>
        </header>
        ...
        <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          <div className={panelClass}>
            ...
            <div className="overflow-hidden rounded-xl border border-zinc-200/80 shadow-sm dark:border-zinc-800/80">
              <div className="grid grid-cols-[1.4fr_1.2fr_1fr_0.9fr_0.8fr] gap-3 bg-zinc-100 px-4 py-3 text-xs font-semibold uppercase tracking-[0.08em] text-zinc-600 dark:bg-zinc-900 dark:text-zinc-300">
                <span>Nama</span>
                <span>Email</span>
                <span>Telepon</span>
                <span>Status</span>
              ...
              {customers.length === 0 ? (
                <div className="px-4 py-6 text-sm text-zinc-500 dark:text-zinc-400">
                  Belum ada data. Gunakan tombol Refresh atau buat customer baru.
                </div>
              ) : (
                <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                  {customers.map((customer) => (
                    <div
                      key={customer.id}
                      className={cn(
                        "grid grid-cols-[1.4fr_1.2fr_1fr_0.9fr_0.8fr] items-center gap-3 px-4 py-3 text-sm",
                    ...
                    >
                      ...
                      <div className="flex items-center justify-end gap-2 text-xs">
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => handleRefreshRow(customer.id)}
            ...
            <form className="space-y-3" onSubmit={handleSubmit}>
              ...
              <div className="space-y-1">
                ...
                <input
                  className={inputClass}
                  type="email"
                  placeholder="customer@contoh.id"
                  required
                  value={form.email}
                  onChange={(event) =>
              ...
              <div className="flex items-center gap-3">
                ...
                {!isEditing ? (
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => setForm({ id: "", name: "", email: "", phone: "", status: "" })}
                  >
                    Reset form
                  </Button>
                ) : null}
